...
contract GMIERC721R80 is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard, FixedPoint {
    ...
    constructor(
        ...
        require(mintPrice_ > 1 ether, "Mint price should be more than 1 NRG");
    ...
    function _mintTokens(
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            uint256 nextTokenId = totalMints + 1;
            _safeMint(_to, nextTokenId);
            refundPrice[nextTokenId] = (_price * 80) / 100;
            if (((_price * 20) / 100) > 0) {
                _transferNRG(owner(), (_price * 20) / 100);
            }
            totalMints += 1;
        }
    }
