...
contract GMIERC721R80C is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard, FixedPoint, OperatorFilter {
    ...
    modifier mintCompliance(uint256 _quantity, uint256 _maxTxMintAmount) {
    ...
    function mint(uint256 _quantity) external payable whenNotPaused noContracts nonReentrant {
        ...
        require(numberMinted[msg.sender] + _quantity <= maxUserMintAmount, "Max amount");

        bool onlyRemint = totalMints == maxMintSupply && refundedTokenIds.length > 0 ? true : false;

        if (totalMints == maxMintSupply && refundedTokenIds.length > 0) {
            require(msg.value == ((mintPrice * 80) / 100) * _quantity, "Bad value");

            _reMintTokens(msg.sender, _quantity, mintPrice, maxTxMintAmount);
        }

        if (totalMints + _quantity > maxMintSupply && refundedTokenIds.length > 0 && !onlyRemint) {
            uint256 totalNFTCount = totalMints + _quantity;
            uint256 remintsRequired = totalNFTCount - maxMintSupply;
            uint256 freshMintsRequired = _quantity - remintsRequired;

            uint256 remintValue = ((mintPrice * 80) / 100) * remintsRequired;
            uint256 freshMintValue = mintPrice * freshMintsRequired;

            require(msg.value == remintValue + freshMintValue, "Bad value");

            if (remintsRequired > 0) {
                _reMintTokens(msg.sender, remintsRequired, mintPrice, maxTxMintAmount);
            }

            if (freshMintsRequired > 0) {
                _mintTokens(msg.sender, freshMintsRequired, mintPrice, maxTxMintAmount);
            }
        }

        if (totalMints + _quantity <= maxMintSupply && !onlyRemint) {
            require(msg.value == _quantity * mintPrice, "Bad value");

            _mintTokens(msg.sender, _quantity, mintPrice, maxTxMintAmount);
        }
    ...
    function presaleMint(uint256 _quantity) external payable whenNotPaused noContracts nonReentrant {
        ...
        require(mintedAmount + _quantity <= maxMintSupply, "Max supply");
        require(presaleNumberMinted[msg.sender] + _quantity <= whitelists[msg.sender], "Max amount");
        ...
        if (onlyRemint) {
            require(msg.value == ((presaleMintPrice * 80) / 100) * _quantity, "Bad value");

            _reMintTokens(msg.sender, _quantity, presaleMintPrice, presaleMaxTxMintAmount);
        }
        ...
        if (totalMints + _quantity > maxMintSupply && refundedTokenIds.length > 0 && !onlyRemint) {
            ...
            uint256 remintValue = ((presaleMintPrice * 80) / 100) * remintsRequired;
            uint256 freshMintValue = presaleMintPrice * freshMintsRequired;

            require(msg.value == remintValue + freshMintValue, "Bad value");

            if (remintsRequired > 0) {
                _reMintTokens(msg.sender, remintsRequired, presaleMintPrice, presaleMaxTxMintAmount);
            }

            if (freshMintsRequired > 0) {
                _mintTokens(msg.sender, freshMintsRequired, presaleMintPrice, presaleMaxTxMintAmount);
            }
        }

        if (totalMints + _quantity <= maxMintSupply && !onlyRemint) {
            require(msg.value == _quantity * presaleMintPrice, "Bad value");

            _mintTokens(msg.sender, _quantity, presaleMintPrice, presaleMaxTxMintAmount);
        }
    ...
    function _removeTokenId(uint256 _index) internal {
        uint256 lastIndex = refundedTokenIds.length - 1;
        if (_index != lastIndex) {
            refundedTokenIds[_index] = refundedTokenIds[lastIndex];
        }
    ...
    function _mintTokens(
        ...
        uint256 _price,
        uint256 _maxTxMintAmount
    ) internal mintCompliance(_quantity, _maxTxMintAmount) {
        mintedAmount += _quantity;

        for (uint256 i = 0; i < _quantity; i++) {
            uint256 nextTokenId = totalMints + 1;
            _safeMint(_to, nextTokenId);
            uint256 cut80 = (_price * 80) / 100;
            refundPrice[nextTokenId] = cut80;
            uint256 remainingAmount = (_price * 20) / 100;
            if (remainingAmount > 0) {
                _transferNRG(owner(), remainingAmount);
                _transferNRG(operatorRegistry.fundReceiver(), (cut80 * operatorRegistry.sharePercentageBps()) / 10000);
            }
            totalMints += 1;
        }
    }
    ...
    /**
     * @dev Internal function to re-mint NFTs
     */
    function _reMintTokens(
        address _to,
        uint256 _quantity,
        uint256 _price,
        uint256 _maxTxMintAmount
        ...
        mintedAmount += _quantity;
        require(_quantity <= refundedTokenIds.length, "Quantity exceeds remints");

        for (uint j = 0; j < _quantity; j++) {
            _safeMint(_to, refundedTokenIds[0]);
            refundPrice[refundedTokenIds[0]] = (_price * 80) / 100;
            _removeTokenId(0);
        }
    }
