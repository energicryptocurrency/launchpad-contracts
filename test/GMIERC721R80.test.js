...
describe("GMIERC721R80 test", async () => {
  let gmiERC721R80;
  ...
  beforeEach(async () => {
    [owner, minter1, minter2, minter3, minter4, stranger, ...addrs] = await ethers.getSigners();

    gmiERC721R80 = await ethers.deployContract("GMIERC721R80", [
      name,
      symbol,
      baseURI,
      maxMintSupply,
    ...
    gmiERC721R80FreePresale = await ethers.deployContract("GMIERC721R80", [
      name,
      symbol,
      baseURI,
      maxMintSupply,
  ...
  describe("presaleMint()", () => {
    ...
    it("should mint", async () => {
      ...
      expect(balanceOfContract).to.be.equal("800000000000000000");
      const balance = await gmiERC721R80.balanceOf(minter1.address);
      expect(balance).to.be.equal(1);
