...
describe("GMIERC721C test", async () => {
  let gmiERC721;
  let owner;
  let minter1;
  let minter2;
  ...
  beforeEach(async () => {
    [owner, minter1, minter2, stranger, fundReceiver, ...addrs] = await ethers.getSigners();
    ...
    gmiERC721 = await ethers.deployContract("GMIERC721C", [
      name,
      symbol,
      baseURI,
      maxMintSupply,
  ...
  describe("deployment and constructor", () => {
    ...
    it("should return operatorRegistry", async () => {
      const result = await gmiERC721.operatorRegistry();
      expect(result).to.be.equal(await operatorRegistryProxy.getAddress());
    });
  });
  ...
  describe("transfers", () => {
    ...
    beforeEach(async () => {
      ...
      await gmiERC721.ownerMint(minter1.address, 1);
      await gmiERC721.ownerMint(stranger.address, 1);
    });
    ...
    it("should transfer NFT to whitelisted contract", async () => {
      ...
      await gmiERC721
        .connect(minter1)
        .transferFrom(await minter1.getAddress(), await nftReceiver.getAddress(), 1);
    });
    ...
    it("should send to EOA", async () => {
      await gmiERC721
        .connect(minter1)
        .transferFrom(await minter1.getAddress(), await minter2.getAddress(), 1);
    });
  });
});
